...
struct grSEmitter
{
	...
	{
		// This could be done smoother; NewSpawns = dt * SpawnRate + scaler
		// Problem is I don't know how to make the spawnrate API call make sense in relation to real time, for example seconds as it is now
		SpawnAccT += deltaT;
		...
		if ( NewSpawns > 0 )
		{
			...
			endIdx = grMath::Min( rParticle->Alive + NewSpawns + 1, MaxParticleSize - 1 );

			// Increment particles alive
			rParticle->Alive += NewSpawns + 1;	// TODO: +1 should be needed so find bug
...
struct grSLifeUpdater
{
	...
	void Kill( pU<grSParticle>& rParticle, const sizeT nowIdx )
	{
		...
		// Values that are generated with operator += needs resetting.
