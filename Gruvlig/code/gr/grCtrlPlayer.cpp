...
grCtrlPlayer::Update(void)
{
	if(GetActiveStatus() == true)
	{
		...
		if(input.GetKey(sf::Keyboard::Key::W))	m_InputDir.y -= 1.0f;
		if(input.GetKey(sf::Keyboard::Key::A))	m_InputDir.x -= 1.0f;
		if(input.GetKey(sf::Keyboard::Key::S))	m_InputDir.y += 1.0f;
		if(input.GetKey(sf::Keyboard::Key::D))	m_InputDir.x += 1.0f;
